...
def get_version():
    """从 /opt/linuxwave_config/VERSION.json 获取主程序版本号"""
...
class LinuxWaveCLI:
    def __init__(self):
        ...
        self.verbose = False
        self._logger = logging.getLogger("LinuxWave")
    ...
    def _create_parser(self):
        parser = argparse.ArgumentParser(
            prog="wave",
            description=f"LinuxWave {self.version}\nA package manager for Linux jailbreak developers.",
            formatter_class=argparse.RawDescriptionHelpFormatter,
            usage="wave <command> [package] [flags]",
            epilog="For more details, visit: https://linuxwave.org",
            add_help=False
        )

        parser.add_argument('-h', '--help', action='store_true', help='show this help message and exit')
        parser.add_argument('-V', '--version', action='version', version=f'LinuxWave {self.version} 🌊')
    ...
    def handle_upgrade(self, args):
        ...
        if self._is_protected(safe_name):
            ...
            print(f"{YELLOW}🌊 To update LinuxWave, download the new version manually:{RESET}")
            print(f"{YELLOW}🌊   curl -fsSL -o {INSTALL_DIR}/wave https://raw.githubusercontent.com/Sha0huaZhang/LinuxWave/main/wave.py{RESET}")
            return
